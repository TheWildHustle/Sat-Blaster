...
__export(src_exports, {
  checkEventMatchesFilter: () => checkEventMatchesFilter,
  db: () => db,
...
var Database = class extends import_dexie.default {
  users;
  events;
  eventTags;
  nip05;
...
var NDKCacheAdapterDexie = class {
  ...
  constructor(opts = {}) {
    ...
    this.debug = opts.debug || (0, import_debug.default)("ndk:dexie-adapter");
  ...
  async setEvent(event, filters, relay) {
    if (event.kind === 0) {
      if (!this.profiles)
        return;
      try {
        const profile = (0, import_ndk.profileFromEvent)(event);
...
function checkEventMatchesFilter(event, filter) {
  ...
  try {
    deserializedEvent = (0, import_ndk.deserialize)(event.event);
    if (!(0, import_nostr_tools.matchFilter)(filter, deserializedEvent))
      return;
  } catch (e) {
    console.log("failed to parse event", e);
    return;
  }
  const ndkEvent = new import_ndk.NDKEvent(void 0, deserializedEvent);
  const relay = event.relay ? new import_ndk.NDKRelay(event.relay) : void 0;
...
function foundEvent(subscription, event, relayUrl, filter) {
  try {
    const deserializedEvent = (0, import_ndk.deserialize)(event.event);
    if (filter && !(0, import_nostr_tools.matchFilter)(filter, deserializedEvent))
      return;
    const ndkEvent = new import_ndk.NDKEvent(void 0, deserializedEvent);
